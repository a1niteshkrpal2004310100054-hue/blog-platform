...
import { clearUploads } from "@/redux/blogSlice";
...
const CreateBlog = () => {
  ...
  const uploadedImages = useAppSelector((state) => state.blog.uploadedImages);
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    ...
    formData.append(
      "allUploadedImages",
      JSON.stringify(Array.from(uploadedImages))
    );
    if (data.image) {
    ...
    try {
      ...
      toast.success("Form Submitted", res.data);
  ...
  return (
    <>
      <nav className="mx-auto max-w-[800px] w-full flex items-center justify-between p-2">
      ...
      >
        ...
        {/* Image */}
        <div className="relative border-4 bg-white border-gray mt-10 overflow-hidden">
          <Label
            htmlFor="uploadbanner"
            className="h-[50vh] block cursor-pointer"
          >
            <img
              src={preview}
              alt="preview"
